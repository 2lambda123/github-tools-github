...
/**
 * Respository encapsulates the functionality to create, query, and modify
 * files.
 */
class Repository extends Requestable {
  ...
  constructor(fullname, auth, apiBase) {
    ...
    this.__currentTree = {branch : null, sha : null};
  }
   ...
   * @param {Requestable.callback} [cb] - will receive true if the request is
   *     successful
   * @return {Promise} - the promise for the http request
   */
  deleteRef(ref, cb) {
    return this._request('DELETE', `/repos/${this.__fullname}/git/refs/${ref}`,
                         null, cb);
  }
  ...
  getPullRequest(number, cb) {
    return this._request('GET', `/repos/${this.__fullname}/pulls/${number}`,
                         null, cb);
  }
   ...
   * @param {Requestable.callback} [cb] - will receive the list of files from
   *     the API
   * @return {Promise} - the promise for the http request
   */
  listPullRequestFiles(number, cb) {
    return this._request(
        'GET', `/repos/${this.__fullname}/pulls/${number}/files`, null, cb);
  }
  ...
  compareBranches(base, head, cb) {
    return this._request(
        'GET', `/repos/${this.__fullname}/compare/${base}...${head}`, null, cb);
  }
   ...
   */
  getBlob(sha, cb) {
    return this._request('GET', `/repos/${this.__fullname}/git/blobs/${sha}`,
                         null, cb, 'raw');
  }
   ...
   */
  getCommit(sha, cb) {
    return this._request('GET', `/repos/${this.__fullname}/git/commits/${sha}`,
                         null, cb);
  }

  /**
   * List the commits on a repository, optionally filtering by path, author or
   * time range
   * @see
   ...
   * @param {(Date|string)} [options.since] - only commits after this date will
   *     be returned
   * @param {(Date|string)} [options.until] - only commits before this date will
   *     be returned
   * @param {Requestable.callback} cb - will receive the list of commits found
  ...
  getSingleCommit(ref, cb) {
    ref = ref || '';
    return this._request('GET', `/repos/${this.__fullname}/commits/${ref}`,
                         null, cb);
  }

  /**
   * Get tha sha for a particular object in the repository. This is a
   * convenience function
   * @see https://developer.github.com/v3/repos/contents/#get-contents
   * @param {string} [branch] - the branch to look in, or the repository's
   *     default branch if omitted
   * @param {string} path - the path of the file or directory
   * @param {Requestable.callback} cb - will receive a description of the
   *     requested object, including a `SHA` property
   * @return {Promise} - the promise for the http request
   */
  getSha(branch, path, cb) {
    branch = branch ? `?ref=${branch}` : '';
    return this._request(
        'GET', `/repos/${this.__fullname}/contents/${path}${branch}`, null, cb);
  }
  ...
  listStatuses(sha, cb) {
    return this._request(
        'GET', `/repos/${this.__fullname}/commits/${sha}/statuses`, null, cb);
  }
  ...
  getTree(treeSHA, cb) {
    return this._request(
        'GET', `/repos/${this.__fullname}/git/trees/${treeSHA}`, null, cb);
  }
  ...
  _getContentObject(content) {
    if (typeof content === 'string') {
      ...
      return {content : Utf8.encode(content), encoding : 'utf-8'};
      ...
      return {content : Base64.encode(content), encoding : 'base64'};

    } else { // eslint-disable-line
      log(`Not sure what this content is: ${typeof content}, ${
  ...
  updateTree(baseTreeSHA, path, blobSHA, cb) {
    let newTree = {
      ...
      tree : [ {path : path, sha : blobSHA, mode : '100644', type : 'blob'} ]
    };
  ...
  commit(parent, tree, message, cb) {
    let data = {message, tree, parents : [ parent ]};

    return this
        ._request('POST', `/repos/${this.__fullname}/git/commits`, data, cb)
        .then((response) => {
  ...
  updateHead(ref, commitSHA, force, cb) {
    return this._request('PATCH', `/repos/${this.__fullname}/git/refs/${ref}`,
                         {sha : commitSHA, force : force}, cb);
  }

  /**
   * Get information about the repository
   * @see https://developer.github.com/v3/repos/#get
   * @param {Requestable.callback} cb - will receive the information about the
   *     repository
   * @return {Promise} - the promise for the http request
   */
  getDetails(cb) {
   ...
   */
  getContributors(cb) {
    return this._request('GET', `/repos/${this.__fullname}/stats/contributors`,
                         null, cb);
  }

  /**
   * List the users who are collaborators on the repository. The currently
   * authenticated user must have push access to use this method
   * @see
   ...
   */
  getCollaborators(cb) {
    return this._request('GET', `/repos/${this.__fullname}/collaborators`, null,
                         cb);
  }
   ...
   * @param {Requestable.callback} cb - will receive true if the user is a
   *     collaborator and false if they are not
   * @return {Promise} - the promise for the http request {Boolean}
   *     [description]
   */
  isCollaborator(username, cb) {
    return this._request(
        'GET', `/repos/${this.__fullname}/collaborators/${username}`, null, cb);
  }
   ...
   * @param {boolean} raw - `true` if the results should be returned raw instead
   *     of GitHub's normalized format
  ...
  getReadme(ref, raw, cb) {
    return this._request('GET', `/repos/${this.__fullname}/readme`, {ref}, cb,
                         raw);
  }
   ...
   * @param {Object} options - the pull request description
   * @param {Requestable.callback} [cb] - will receive the pull request
   *     information
   * @return {Promise} - the promise for the http request
   */
  updatePullRequst(number, options, cb) {
    return this._request('PATCH', `/repos/${this.__fullname}/pulls/${number}`,
                         options, cb);
  }
  ...
  updateHook(id, options, cb) {
    return this._request('PATCH', `/repos/${this.__fullname}/hooks/${id}`,
                         options, cb);
  }
   ...
   * @param {Requestable.callback} cb - will receive true if the call is
   *     successful
   * @return {Promise} - the promise for the http request
   */
  deleteHook(id, cb) {
   ...
   * @param {string} branch - the branch to delete from, or the default branch
   *     if not specified
   * @param {string} path - the path of the file to remove
   * @param {Requestable.callback} cb - will receive the commit in which the
   *     delete occurred
   * @return {Promise} - the promise for the http request
   */
   ...
   * @param {string} branch - the branch to carry out the reference change, or
  ...
  move(branch, oldPath, newPath, cb) {
    ...
    return this.getRef(`heads/${branch}`)
        ...
        .then(({data : tree}) => this.commit(
                  oldSha, tree.sha, `Renamed '${oldPath}' to '${newPath}'`))
        .then(({data : commit}) =>
   ...
   * @param {boolean} [options.encode] - true if the content should be base64
   *     encoded
    ...
    return this.getSha(branch, filePath)
        .then(
            (response) => {
              ...
              return this._request(
                  'PUT', `/repos/${this.__fullname}/contents/${filePath}`,
                  commit, cb);
            },
            () => {
              return this._request(
   ...
   * @param {Requestable.callback} cb - will receive true if the repository is
   *     starred and false if the repository
   *                                  is not starred
   * @return {Promise} - the promise for the http request {Boolean}
   *     [description]
   */
  ...
  updateRelease(id, options, cb) {
    return this._request('PATCH', `/repos/${this.__fullname}/releases/${id}`,
                         options, cb);
  }
   ...
   */
  getRelease(id, cb) {
    return this._request('GET', `/repos/${this.__fullname}/releases/${id}`,
                         null, cb);
  }
   ...
   * @param {Requestable.callback} cb - will receive true if the operation is
   *     successful
   * @return {Promise} - the promise for the http request
   */
  deleteRelease(id, cb) {
    return this._request('DELETE', `/repos/${this.__fullname}/releases/${id}`,
                         null, cb);
  }
   ...
   * @param {Requestable.callback} [cb] - will receive the merge information if
   *     the operation is successful
   * @return {Promise} - the promise for the http request
   */
  mergePullRequest(number, options, cb) {
    return this._request(
        'PUT', `/repos/${this.__fullname}/pulls/${number}/merge`, options, cb);
  }
}
